...
function songSize($sizeMusic) {
  if($sizeMusic > 0 && $sizeMusic < 1024){
    return $sizeMusic . " b";
  }
  elseif ($sizeMusic > 1023 && $sizeMusic < 1048576){
    return round($sizeMusic/1024, 2) . " kb";
  }
  elseif ($sizeMusic > 1048577) {
    return round($sizeMusic/1048576, 2) . " mb";
  }
}
    ...
    <div id="listarea">
      <ul id="musiclist">
        ...
        foreach ( $songs as $song) {
          if(strstr($song, ".mp3")){
          ?>
          <li class="mp3item" ><a href="<?= 'songs/'.basename($song)?> "> <?= basename($song) ?></a> (<?= songSize(sizeMusic("songs/". basename($song))) ?>)</li>
        <?php } }
